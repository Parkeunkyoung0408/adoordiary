...
import { Edit3, Trash2, Archive, Settings } from "lucide-react";
import { MaeumProvider, useMaeum } from "./maeum/MaeumContext";

const NAV_ITEMS = [
  { href: "/edit", label: "기록", icon: Edit3 },
  { href: "/bucket", label: "버킷", icon: Trash2 },
    ...
    <div className="flex-1 min-h-0 w-full bg-[var(--bg-container)] flex flex-col relative overflow-hidden transition-all duration-300">
      ...
      <div className="shrink-0 px-5 pt-6 pb-3 flex items-center justify-between border-b border-[var(--border-color)] bg-[var(--bg-container)]/80 backdrop-blur-md z-10 transition-colors duration-300">
        <div className="flex items-center gap-2">
          <Link href="/edit" aria-label="마음써방 홈">
            <svg
          ...
          {NAV_ITEMS.map((tab) => {
            ...
            return (
              <Link
                ...
                style={{ minWidth: "64px" }}
              >
                <IconComponent className="w-[20px] h-[20px]" strokeWidth={isActive ? 2.5 : 2} />
